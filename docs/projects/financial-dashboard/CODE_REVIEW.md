...
6. **`defaults.js` line 97:** `note: 'Company workplace pension. Includes transferred People\'s Pension...'` — backslash escaping works but is fragile when the string is interpolated into HTML without `escHtml()`. The `pension.note` value is directly injected into innerHTML in `assets.js` line 27 and `ui.js` line 563 without sanitisation.
