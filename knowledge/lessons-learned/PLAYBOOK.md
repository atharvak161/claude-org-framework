...
- **Check the README on every repo change, however small.** A repo's README goes stale silently, and a stale README is worse than none: it sends people to configure things that no longer exist. JobScope's README advertised the Jooble job source, and an API key to request for it, three PRs after Jooble was deleted from the code. Cross-check `.env.example` against the env table specifically — those two drift apart fastest. State the result as one line in the completion report (`README: checked, no update needed` or `README: checked, updated — <what>`); silence reads as not-checked and `DONE` is not available without it. Full procedure: knowledge/protocols/WORKFLOW_PROTOCOL.md, "README currency gate".
